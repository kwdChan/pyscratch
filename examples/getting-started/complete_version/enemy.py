...
# clone movement
def clone_movement(clone_sprite: pysc.Sprite): # same as `def clone_movement(clone_sprite):`

    clone_sprite.show()

    # random height
    clone_sprite.y = pysc.random_number(0, pysc.game['screen_height'])
    ...
    if pysc.random_number(0, 1) > 0.5: 
        ...
        clone_sprite.x = pysc.game['screen_width']
    ...
    player: pysc.Sprite = pysc.game['player'] # same as `player = pysc.game['player']`
    ...
    while True:
        ...
        if clone_sprite.distance_to_sprite(player) < (100+player['size']*100):
