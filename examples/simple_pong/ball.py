...
def movement():
    ...
    while True: 
        ...
        if pysc.game.shared_data['running']:
            ...
            if pysc._is_touching(ball_sprite, top_edge):
                speed_y = abs(speed_y)

            if pysc._is_touching( ball_sprite, bottom_edge):
                speed_y = -abs(speed_y)

            if pysc._is_touching(ball_sprite, left_edge):
                speed_x = abs(speed_x)
                ball_sprite.broadcast_message('right_score', None)

            if pysc._is_touching( ball_sprite, right_edge):
                speed_x = -abs(speed_x)
                ball_sprite.broadcast_message('left_score', None)

            if pysc._is_touching( ball_sprite, pysc.game.shared_data['right_paddle']):
                speed_x = -abs(speed_x)
                pysc.game.play_sound('bong')

            if pysc._is_touching( ball_sprite, pysc.game.shared_data['left_paddle']):
                speed_x = abs(speed_x)
                pysc.game.play_sound('bong')
