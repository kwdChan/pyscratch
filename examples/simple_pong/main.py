...
left_paddle_sprite.set_collision_type(1)


## behaviour
## - move by key 'a' and 'd' in a limited space

left_timer_event = pysc.game.create_timer_trigger(10)

def check_move_left(n):
    movement = 0
...
def spawn_ball(): 
    ...
    ball_sprite.body.velocity = (vx, vy)
...
# Score display
score_left = rect_sprite((170, 170, 170), 100, 50, pos=(100, SCREEN_HEIGHT//2))
pysc.game.add_sprite(score_left)

score_right = rect_sprite((170, 170, 170), 100, 50, pos=(SCREEN_WIDTH-100, SCREEN_HEIGHT//2))
...
pysc.game.boardcast_message('restart', None) # the message can passes data to the callback function
